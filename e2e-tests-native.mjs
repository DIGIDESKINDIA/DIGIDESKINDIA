#!/usr/bin/env node
/**
 * End-to-End API Testing for DigiDesk Tools - Native FormData Version
...
/**
 * Test PDF Tool using native FormData
 */
async function testPdfTool(toolName, testConfig) {
  ...
  try {
    ...
    form.append('file', blob, path.basename(inputFile));
    ...
    if (!response.ok) {
      ...
      try {
        ...
        result.error = json.message || json.error || text.substring(0, 100);
      } catch {
    ...
    if (contentType.includes('application/json')) {
    ...
    } else if (contentType.includes('application/pdf') || contentType.includes('zip') || contentType.includes('octet-stream')) {
      // Binary response
      const buffer = await response.arrayBuffer();
      const uint8 = new Uint8Array(buffer);
      result.size_output = (uint8.length / (1024 * 1024)).toFixed(2);
      ...
      // Validate output
      if (validationFn(uint8)) {
        result.validation = 'PASS';
        result.status = 'PASS';
      } else {
        result.validation = 'FAIL';
        result.error = 'Output validation failed - invalid file format';
        result.status = 'FAIL';
      }
    } else {
      result.output = 'PASS'; // Accept other content types as potentially valid
      result.status = 'PASS';
    }
  } catch (error) {
...
async function testImageTool(toolName, testConfig) {
  ...
  try {
    ...
    if (inputFile.endsWith('.jpg') || inputFile.endsWith('.jpeg')) {
      mimeType = 'image/jpeg';
    } else if (inputFile.endsWith('.webp')) {
      mimeType = 'image/webp';
    }
    ...
    if (contentType.includes('application/json')) {
      ...
      if (uint8.length === 0) {
        ...
        result.output = 'PASS';
        result.validation = validationFn(uint8, contentType.split('/')[1]) ? 'PASS' : 'FAIL';
...
async function runAllTests() {
  console.log('\n' + '='.repeat(80));
  console.log('DigiDesk India - E2E API Test Suite (Native FormData)');
  ...
  for (const test of pdfTests) {
    try {
      ...
      process.stdout.write(`${result.status === 'PASS' ? '✓' : '✗'} ${test.name}: ${result.status}`);
      if (result.error) console.log(` (${result.error.substring(0, 50)})`);
      else console.log();
    } catch (error) {
